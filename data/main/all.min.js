...
function successSendButton(classNm, msg=true) {
    if(msg) messageSee("Pomyślnie wysłano żądanie", "success");
...
function errorSendButton(classNm, msg=true) {
    if(msg) messageSee("Nie udało się wysłać żądania", "failed");
    $(classNm).removeClass("animate-spin");
    $(classNm).addClass("animate-withoutSpin");
...
function onMessage(event) {
    ...
    if (data.what == "power-now") {
        ...
        $(".status .sts-list .pin_" + data.pinID + " span").next().text(data.action ? "Włączone" : "Wyłączone");
...
function planDataSend(action, pinId, data, hours) {
    ...
    $.ajax({
        ...
        if(err.status == 302 && err.responseText == "FAILED - action exist") messageSee("Taka akcja już istnieje", "failed")
        else messageSee("Nie udało się wysłać żądania", "failed")
    }).done(function (data) {
        successSendButton(_thisSubBtn);
        clearTimeout(buttonSubmitTO)
         messageSee("Pomyślnie wykonano żądanie", "success")
            ...
            <div class="restart">
                <div class="cnt">
                    <div class="restart-power">
                        <button class="button-sbm" id="restart-btn">Uruchom ponownie urządzenie</button>
                    </div>
                    <input type="password" name="password" id="rest-pass" placeholder="Hasło">
                    <div class="resetuj-divice">
                        <input type="checkbox" name="check" id="check">
                        <button class="button-sbm" id="reset-btn" disabled>Resetuj urządzenie do ustawień fabrycznych</button>
                    </div>
                </div>
            </div>
        </div>
...
$('body').delegate('#restart-btn', 'click', function(event){
    event.preventDefault();
    if($('#rest-pass').val().length == 0) return alert("Nie podano hasła");
    if(!confirm("Czy na pewno chcesz uruchomić ponownie urządzenie?")) return;
    $.ajax({
        url: '/restart',
        method: 'POST',
        data: {
        ...
        success: function(data){
            console.log(data);
            messageSee("Restart urządzenia w toku...", "success");
        }
    });
});
...
$('body').delegate('#reset-btn', 'click', function(event){
    event.preventDefault();
    if($('#rest-pass').val().length == 0) return alert("Nie podano hasła");
    if(!confirm("Czy na pewno chcesz przywrucić urządzenie do ustawień fabrycznych?")) return;
    $.ajax({
        url: '/factoryreset',
        method: 'POST',
        data: {
        ...
        success: function(data){
            console.log(data);
            messageSee("Przywracanie do ustawień fabrycznych urządzenia w toku...", "success");
        }
    });
});